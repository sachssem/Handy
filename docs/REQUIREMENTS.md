...
- ✅🍴 **Sprach-Allowlist für Auto-Detect** (gegen LID-Ausreißer wie Kyrillisch bei kurzen englischen Phrasen): erkannte Sprache außerhalb der Liste — bei Archs ohne Sprach-Report (Parakeet) ersatzweise die dominante Schrift des Outputs — → einmaliger Retry mit auf die Primärsprache gepinnter Konditionierung (`language_allowlist`; UI unter der Sprachauswahl, nur bei „Auto").
  _Gemessene Grenze (2026-07-12):_ Parakeets Pin ist nur weiche Konditionierung — beim realen Kyrillisch-Fall („feat slash whatsapp dash lifecycle") liefern de-Pin, en-Pin und auto identisches Kyrillisch; Qwen3-ASR 1.7B transkribiert dieselbe Aufnahme korrekt lateinisch („Feed slash WhatsApp dash lifecycle"). Für englisch-lastiges Tech-Diktat bleibt Qwen das richtige Standard-Modell (siehe Modell-Entscheid unten); der Guard fängt es bei Whisper-Archs und macht den Fall im Log sichtbar.
  _Fallback-Modell (Eskalation):_ Optional lässt sich unter der Allowlist ein zweites, heruntergeladenes Modell wählen (`language_allowlist_fallback_model`). Ist es gesetzt und vom aktiven Modell verschieden, überspringt der Guard bei einem Ausreißer den wirkungslosen Pin-Retry und transkribiert dieselbe Aufnahme einmal mit dem Fallback-Modell (Auto-Sprache, Hint nur wenn dessen Arch ihn annimmt) — z. B. Parakeet-Alltag mit Qwen3-ASR als Rettungsanker. Das Ergebnis wird nur übernommen, wenn es wieder in der Allowlist liegt; sonst (oder bei Lade-/Laufzeitfehler) bleibt die Originaltranskription, ohne weitere Kaskade. Das Fallback-Modell wird nur kurz geladen und sofort wieder verworfen (kurzer RAM-Peak, Cache/Unload-Timer unberührt).
...
- ✅🍴 **Auto-Lernen aus Korrekturen** (Wispr-Flow-Loop: manuelle Nachkorrekturen im Textfeld beobachten → Dictionary automatisch füttern) — im Fork umgesetzt: macOS-Lern-Loop (Accessibility-Read nach Paste mit Element-Pinning, jetzt event-getrieben via AXObserver mit 2-s-Poll als Fallback, Wort-Diff, Anti-Vergiftungs-Gates inkl. Phonetik), Review-UI und „Gelernt"-Toast mit Undo, Trial-Mode. Design/Details: `docs/design/auto-learn-corrections.md`.
